...
public class SceneBootstrap : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        // 1. Восстанавливаем инвентарь нод
        if (inventoryManager != null)
            gsm.RestoreInventory(inventoryManager);
        Debug.Log("[SceneBootstrap] восстановили инвентарь нод");

        // НОВОЕ: Восстанавливаем обычный инвентарь предметов
        var itemMgr = FindAnyObjectByType<InventorySlotManager>();
        if (itemMgr != null)
        {
            gsm.RestoreItemInventory(itemMgr);
            Debug.Log("[SceneBootstrap] восстановили обычный инвентарь предметов");
        }
        ...
        // 5. Запускаем поиск спавн-поинта на сцене по сохраненному ID и телепортируем игрока
        Debug.Log("[SceneBootstrap] запускаем телепортацию");
        StartCoroutine(SpawnPlayerAtSpawnPoint());
    }
